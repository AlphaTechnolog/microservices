...
import { missingProduct as missingProductType, type MissingProductType } from './schemas/missingProduct';
...
async function consumeEvents() {
  ...
  await consumer.run({
    eachMessage: async ({ message }) => {
      try {
        const missingProduct: MissingProductType = missingProductType.fromBuffer(message.value ?? "{}");
